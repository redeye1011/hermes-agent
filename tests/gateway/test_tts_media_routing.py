...
@pytest.mark.asyncio
async def test_streaming_delivery_dedupes_media_tag_and_bare_photon_audio(tmp_path, monkeypatch):
    """A streamed response must not deliver the same audio tag/path twice."""
    media_file = _allowed_media_path(tmp_path, monkeypatch, "speech.mp3")
    event = MessageEvent(
        text="make speech",
        message_type=MessageType.TEXT,
        source=SessionSource(platform=Platform("photon"), chat_id="any;-;+155****0000", chat_type="dm"),
        message_id="msg-1",
    )
    adapter = SimpleNamespace(
        name="photon",
    ...
    await GatewayRunner._deliver_media_from_response(
        _fake_runner(None), f"MEDIA:{media_file}\n{media_file}", event, adapter
    )
